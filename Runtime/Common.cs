...
namespace EpicOnlineServiceTransport
{
    public abstract class Common
    {
        ...
        protected Dictionary<PacketKey, List<List<Packet>>> incomingPackets =
            new Dictionary<PacketKey, List<List<Packet>>>();
        ...
        protected Common(EosTransport transport)
        {
            ...
            incomingNotificationId = EOSSDKComponent.GetP2PInterface().AddNotifyPeerConnectionRequest(
            ...
            outgoingNotificationId = EOSSDKComponent.GetP2PInterface().AddNotifyPeerConnectionClosed(
        ...
        {
            var packet = new SendPacketOptions()
            {
            ...
            EOSSDKComponent.GetP2PInterface().SendPacket(ref packet);
        }
        ...
        protected void Send(ProductUserId host, SocketId socketId, byte[] msgBuffer, byte channel)
        {
            ...
            Result result = EOSSDKComponent.GetP2PInterface().SendPacket(ref packet);
        ...
        private bool Receive(out ProductUserId clientProductUserId, out SocketId socketId, out byte[] receiveBuffer,
            byte channel)
        {
            ...
            var outData = new byte[P2PInterface.MaxPacketSize];
            Result result = EOSSDKComponent.GetP2PInterface().ReceivePacket(ref options, out clientProductUserId,
                out socketId, out channel, new ArraySegment<byte>(outData), out var outBytesWritten);
            receiveBuffer = new byte[outBytesWritten];
            Array.Copy(outData, receiveBuffer, outBytesWritten);
