...
namespace EpicOnlineServiceTransport {
    public class Server : Common {
        ...
        public static Server CreateServer(EosTransport transport, int maxConnections) {
            ...
            s.OnReceivedError += (id, exception) => transport.OnServerError.Invoke(id, TransportError.Refused, exception.Message);
        ...
        protected override void OnReceiveInternalData(InternalMessages type, ProductUserId clientUserId, SocketId socketId) {
            ...
            switch (type) {
                case InternalMessages.CONNECT:
                    ...
                    clientUserId.ToString(out var clientUserIdStringU);
                    var clientUserIdString = clientUserIdStringU.ToString();
        ...
        protected override void OnReceiveData(byte[] data, ProductUserId clientUserId, int channel) {
            ...
            if (epicToMirrorIds.TryGetValue(clientUserId, out int connectionId)) {
                ...
                clientUserId.ToString(out var productIdU);
                var productId = productIdU.ToString();
        ...
        public string ServerGetClientAddress(int connectionId) {
            if (epicToMirrorIds.TryGetValue(connectionId, out ProductUserId userId)) {
                userId.ToString(out var userIdStringU);
                var userIdString = userIdStringU.ToString();
                return userIdString;
            } else {
