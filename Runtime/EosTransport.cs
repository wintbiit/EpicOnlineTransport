...
namespace EpicOnlineServiceTransport {
    ...
    /// <summary>
    /// EOS Runtime following the Mirror transport standard
    /// </summary>
    public class EosTransport : Transport {
        ...
        private void Awake() {
            Debug.Assert(Channels != null && Channels.Length > 0, "No channel configured for EOS Runtime.");
            Debug.Assert(Channels.Length < byte.MaxValue, "Too many channels configured for EOS Runtime");

            if(Channels[0] != PacketReliability.ReliableOrdered) {
                Debug.LogWarning("EOS Runtime Channel[0] is not ReliableOrdered, Mirror expects Channel 0 to be ReliableOrdered, only change this if you know what you are doing.");
            }
            if (Channels[1] != PacketReliability.UnreliableUnordered) {
                Debug.LogWarning("EOS Runtime Channel[1] is not UnreliableUnordered, Mirror expects Channel 1 to be UnreliableUnordered, only change this if you know what you are doing.");
            }
        ...
        public override void ClientConnect(string address) {
            ...
            if (ServerActive()) {
                Debug.LogError("Runtime already running as server!");
                return;
            }
        ...
        public override void ServerStart() {
            ...
            if (ClientActive()) {
                Debug.LogError("Runtime already running as client!");
                return;
            }
